...
func TestNewQuery(t *testing.T) {
	...
	expected := fmt.Sprintf(`from(bucket:"") |> range(start:%s,stop:%s) |> filter(fn: (r) => r.host == "host") |> filter(fn: (r) => r._field == "field") |> aggregateWindow(every: 1s, fn: mean, createEmpty: false)`, exampleTimeFmt, exampleTimeFmt)
...
func TestString_Bucket(t *testing.T) {
	expected := `from(bucket:"co2") |> range(start:0001-01-01T00:00:00Z)`
	require.Equal(t, expected, query{bucket: "co2"}.String())
}

func TestString_Range(t *testing.T) {
	expected := fmt.Sprintf(`from(bucket:"") |> range(start:%s)`, exampleTimeFmt)
	require.Equal(t, expected, query{start: exampleTime}.String())
}

func TestString_HostFilter(t *testing.T) {
	expected := `from(bucket:"") |> range(start:0001-01-01T00:00:00Z) |> filter(fn: (r) => r.host == "host")`
	require.Equal(t, expected, query{hosts: []string{"host"}}.String())
}

func TestString_FieldFilter(t *testing.T) {
	expected := `from(bucket:"") |> range(start:0001-01-01T00:00:00Z) |> filter(fn: (r) => r._field == "field")`
	require.Equal(t, expected, query{fields: []string{"field"}}.String())
}

func TestString_AggregateWindow(t *testing.T) {
	expected := `from(bucket:"") |> range(start:0001-01-01T00:00:00Z) |> aggregateWindow(every: 1s, fn: mean, createEmpty: false)`
